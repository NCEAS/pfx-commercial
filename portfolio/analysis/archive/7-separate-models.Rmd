...
dmv <- dm %>% group_by(p_holder) %>%
    ...
    # summarise(cv = sd(log(revenue)-log(revenue.prev), na.rm = T),
      log_specDiv = mean(log(specDiv)),
...
```{r}
library(mgcv)
dmv <- dmv %>% group_by(strategy) %>% mutate(cv_centered = log(cv) - mean(log(cv)))
m1 <- gam(cv_centered ~ s(scaled_log_spec) + s(scaled_log_days), data = dmv)
par(mfrow = c(1,2))
plot(m1)
```
...
```{r}
df = readRDS(file="../data-generated/cfec-diff-for-modeling.rds")
dmv <- df %>% group_by(p_holder) %>%
    summarise(sd_ = sd(log(revenue/revenue.prev)), log_specDiv = mean(log(specDiv)),
    length = mean(log(length+1), na.rm = TRUE), days = mean(log(days+1), na.rm = TRUE),
    strategy = paste(sort(unique(substr(strategy, 1, 1))), collapse = " ")) %>%
  as_data_frame() %>%
  filter(!is.na(sd_)) %>%
  group_by(strategy) %>%
  ...
  mutate(mean_log_spec = mean(log_specDiv)) %>%
  as_data_frame() %>%
  mutate(scaled_log_days = scale(days), scaled_log_spec = scale(log_specDiv),
    scaled_log_length = scale(length))
dmv$strategy <- as.factor(dmv$strategy)

library(mgcv)
dmv <- dmv %>% group_by(strategy) %>% mutate(log_sd = log(sd_) - mean(log(sd_)))
m1 <- gam(log_sd ~ s(scaled_log_spec) + s(scaled_log_days), data = dmv)
par(mfrow = c(1,2))
plot(m1)
