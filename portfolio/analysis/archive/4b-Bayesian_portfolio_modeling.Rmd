...
```{r}
cfecAnnual.nonsalmon = readRDS(file="../data-generated/nonsalmon_linearModeling_complete.rds")
...
N = dim(testData)[1]
b1_cov = cbind(SPEC_DIV) # fixed effect covariate on slope
sigma1_cov = cbind(SPEC_DIV) # fixed effect covariate on slope
b1_cov_re = cbind(SPEC_DIV)
sigma1_cov_re = cbind(SPEC_DIV)

b0_re = cbind(PERMIT) #cbind(P_HOLDER, PERMIT, PERMIT_AREA, PERMIT_SPEC)
b0_re_levels = apply(b0_re,2,max)
b0_re_n = dim(b0_re)[2]

b1_re = cbind(PERMIT)#cbind(P_HOLDER, PERMIT, PERMIT_AREA, PERMIT_SPEC)
b1_re_levels = apply(b1_re,2,max)
b1_re_n = dim(b1_re)[2]

sigma0_re = cbind(PERMIT)
sigma0_re_levels = apply(sigma0_re,2,max)
sigma0_re_n = dim(sigma0_re)[2]

sigma1_re = cbind(PERMIT)
...
list_data = list("N"=N, "K_mean"=K_mean, "b1_cov"=b1_cov, "sigma1_cov"=sigma1_cov,
  ...
  "b0_re"=b0_re, "b0_re_levels"=c(b0_re_levels,0), "b0_re_n"=b0_re_n,
  "b1_re"=b1_re, "b1_re_levels"=c(b1_re_levels,0), "b1_re_n"=b1_re_n,
  "sigma0_re"=sigma0_re, "sigma0_re_levels"=c(sigma0_re_levels,0), "sigma0_re_n"=sigma0_re_n,
  "sigma1_re"=sigma1_re, "sigma1_re_levels"=c(sigma1_re_levels,0), "sigma1_re_n"=sigma1_re_n, "K_sig"=K_sig, "y"=y)
...
stanMod = stan(file = 'portfolio_mixedModel.stan',data = list_data, 
  verbose = TRUE, chains = 1, thin = 1, warmup = 20, iter = 50, 
  pars = list_pars)
