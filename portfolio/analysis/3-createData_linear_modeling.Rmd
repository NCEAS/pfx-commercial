...
permit_rev = group_by(cfecnew, p_holder, year, p_spec) %>%
...
nrow(cfecnew)
cfecnew = cfecnew[-which(paste(cfecnew$p_holder, cfecnew$year, cfecnew$p_spec) %in% 
  paste(permit_rev$p_holder, permit_rev$year, permit_rev$p_spec)), ]
nrow(cfecnew)
...
Calculate gear based strategies c:
