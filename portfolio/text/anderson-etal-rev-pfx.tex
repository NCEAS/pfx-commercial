...
J.C.S. also provided data;
S.C.A. and E.J.W. analyzed the data and wrote the paper with input from all
authors.}
...
management, and
economic factors. Key to managing these industries is identifying
how regulatory actions and individual behaviour affect income variability and, by
extension, economic stability and the sustainable use of natural resources. In
commercial fisheries, communities and vessels fishing a greater diversity of
species have less revenue variability than those fishing fewer species.
...
revenue variability. Here, we evaluate two axes by which fishers in Alaska,
can diversify fishing
activities. We show that, despite increasing specialization over the last 30
years, fishing a set of permits with higher species diversity reduces individual revenue
...
expensive, often limited by regulations such as license limitation or catch share programs, and
...
% TODO AB: Since Table 1 defines all of the fishng strategies that are then used as figure labels (and not defined elsewhere in the text), I think it should go into the main body of the text rather than Suppl Info. This would also help clarify any confusion a reader might have about the difference between a permit strategy and an individual permit.  I suggest 2 additional columns in Table 1: 1) potential species that could be harvested under each permit strategy, 2) number of unique permits contained within each strategy. This would really help to show how much flexibility someone within each permit strategy might have.
...
analyses \cite{vedeld2007, illukpitiya2008, debela2012, kasperski2013, sethi2014, cline2017}, we calculated effective species diversity
for permit holder \(i\) and time (year) \(t\) as the inverse of Simpson's
diversity index \(\lambda\) \cite{simpson1949, jost2006} (also referred to in
...
an indicator variable that takes a value \(0\) if \(\mathrm{div}_{i,t} \ge
  \mathrm{div}_{i,t-1}\) (species diversity increasing year-to-year; generalizing) and takes
a value \(1\) if \(\mathrm{div}_{i,t} < \mathrm{div}_{i,t-1}\) (species
diversity decreasing year-to-year; specializing). Estimating a separate coefficient for the
effect of increasing diversity and decreasing diversity allows the costs and
benefits of these changes to be asymmetric (Fig.~S\figresid).
...
analysis revealed non-stationary trends in revenue (resulting from experience
or other unmodelled factors). This approach eliminates having to deal with the
...
% TODO AH: Should you note that you tested for it?  You say that you assume stationarity after you differnece, but why don’t you test and report it?
We removed
person-year combinations without a subsequent year of revenue.
% TODO AH: what proportion of the data is this?
...
The estimated \(\beta_{j,t}\) coefficients represent years when people with a particular
set of permits tended to collectively do better or worse than the long-term
average (Fig.~S\figstryrs). Therefore, an individual's expected change in
revenue (and variability) represents the expectation after accounting for the
general trend in revenue for that set of permits.
...
% TODO AH: explain here somewhere why gamma 1 &4 are negative
...
ensure all scale parameters, \(\sigma_{i,t}\), are positive. Residuals from the
mean component of our model clearly show a ``V''-shaped pattern, which the
variance component of our model can capture (Fig.~S\figresid). Furthermore, the
...
model the variability as a proxy for revenue risk.
...
a second mixed-effects model representing the variance component. We also
validated the model with simulated data, and were able to recover the true
values (Supporting Information). Code used in our analysis is available at
\url{https://github.com/NCEAS/pfx-commercial/portfolios/}. } \showmatmethods
