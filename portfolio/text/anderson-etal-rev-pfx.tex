...
on individuals for whom commercial fishing formed a substantial part of their
income.
...
group. For salmon permits, we formed salmon strategies based on similarities in
...
analyses {[}\cite{vedeld2007}; \cite{illukpitiya2008}; \cite{debela2012};
\cite{kasperski2013}; sethi2014{]}, we calculated effective species diversity
...
economics as the Herfindahl-Hirschman Index \cite{hirschman1964}) weighted by
revenue \(R\):
...
\noindent where \(s\) indexes species from \(1\) through \(n\). We calculated
...
diversity decreasing; specializing). Estimating a separate coefficient for the
...
The slopes \(\beta_{1,j}\) and \(\beta_{1,j}\) are then allowed to vary by
