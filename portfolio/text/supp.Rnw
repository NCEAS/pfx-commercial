...
Sean C. Anderson \and
Eric J. Ward \and
Andrew O. Shelton \and
Milo D. Adkison \and
Anne H. Beaudreau \and
Richard E. Brenner \and
Alan C. Haynie \and
Jennifer C. Shriver \and
Jordan T. Watson \and
Benjamin C. Williams
}
\date{}
...
For example, someone fishing herring roe in Kodiak (G34K)
...
area ($\sim$ 85\% pink salmon in Prince William Sound to
85\% sockeye salmon on the Alaskan Peninsula).
...
We removed data for pairs of years in which people switched
strategies from one year to the next
...
leaving us with a total of \Sexpr{nstr} (unique or combined) strategies.
...
\section{Model checking with two-stage approach}

We checked our models with a two-stage modelling approach
using the R package lme4 [49].
...
\scriptsize
  \centering
  \caption{Labels and descriptions of unique fishing strategies. In many cases,
a particular strategy represents a group of similar permits across areas
or vessel sizes. The column ``Main species'' indicates species that make up at
least 10\% of the total revenue across all years and individuals for that
fishing strategy. ``Sock'' refers to sockeye salmon.}
  \label{tab:strategies}
  \begin{tabular}{llll}
  \toprule
\input{strategy-table.tex}
  \end{tabular}
\end{table}
\end{landscape}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1\textwidth]{stan-gg-spoke-all.pdf}
\caption{The effect of adding a permit to one's strategy
...
Note that $\mu_{\beta_1}$, $\beta_4$, $\mu_{\gamma_1}$, and $\gamma_4$
have been multiplied by $-1$
to make these coefficients interpretable as effects of
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1.06\textwidth]{stan-str-posteriors-dot.pdf}
\caption{Estimates of within-strategy effects from the hierarchical model.
Panels refer to parameters in equations \ref{eq:mean} and \ref{eq:cv}.
...
Note that $\gamma_{1,j}$ and $\beta_{1,j}$ have been multiplied by $-1$
to make these coefficients interpretable as effects of
increasing specialization (as opposed to decreasing specialization).}
\label{fig:re}
\end{center}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1.05\textwidth]{stan-offset-break-anti-spaghetti.pdf}
\caption{Effects of (A) generalizing and (B) specializing from year-to-year for
  individual permit holders \emph{within} the \Sexpr{nstr} most common
  ...
  Generalizing (A) or specializing (B) is associated with greater expected
  revenue ratios to the right of the zero line and lower expected revenue ratios
  to the left of the zero line. These effects are estimated holding fishing
  season length constant from year-to-year. Dots and line segments represent
  medians and 50\% credible intervals of posteriors. Panel B and Fig.~4 are the
  same.
This is a bivariate display of the $\gamma_{1,j}$, $\gamma_{2,j}$, $\beta_{1,j}$,
  and $\beta_{2,j}$ coefficient estimates displayed in Fig.~\ref{fig:re}.
  Note that $\gamma_{1,j}$ (panel B vertical axis) and $\beta_{1,j}$ (panel B
  horizontal axis) have been multiplied by $-1$ to make these coefficients
  interpretable as effects of increasing specialization (as opposed to
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.6\textwidth]{stan-offset-break-anti-spaghetti-specializing-no-effort.pdf}
\caption{Same as Fig.~4 but without the effort (days fished) predictor and
  without its interaction with species diversity.}
\label{fig:noeffort}
\end{center}
\end{figure}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1\textwidth]{itq-combined.pdf}
\caption{The effect of IFQs (individual fishing quotas) on model estimates. (A)
  Posterior density of the ratio between strategy-level estimates of
  standardized revenue variability ($\gamma_{0j}$'s) after the introduction of
  IFQs vs.\ before the introduction of IFQs. (B) Estimates (medians) of the
  effect of generalizing (increasing species diversity) on variability
  ($\gamma_{1j}$'s) for the full model (``Grouped'') compared to a model where
  separate effects are estimated before and after IFQs were introduced (``IFQ
  split''). Pattern looks similar for the effect of specializing on variability
  ($\gamma_{2j}$'s) and is not shown.}
\label{fig:ifq}
\end{center}
\end{figure}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1\textwidth]{strategy-year-effects.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1\textwidth]{stan-offset-panel-mean-resid.png}
\caption{The log-absolute value residuals from the mean-component model (Eq.~\ref{eq:mean}).
These residuals are modelled in the variance-component model
(Eq.~\ref{eq:cv}).
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1\textwidth]{stan-offset-panel-mean-resid-down.png}
...
% \begin{figure}[htbp]
% \begin{center}
% \includegraphics[width=0.9\textwidth]{lmer-vs-stan2.pdf}
