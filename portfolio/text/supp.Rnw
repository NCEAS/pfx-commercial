...
% TODO: Are you saying that PWS sockeye permits were combined with BB sockeye permits?I can't remember but that's what it sounds like so I just wanted to be clear.
...
using the R package lme4 \citep{bates2015}.
...
\clearpage
% TODO: AB: I suggest 2 additional columns: 1) potential species that could be harvested under each permit strategy, 2) number of unique permits contained within each strategy. This would really help to show how much flexibility someone within each permit strategy might have.
\begin{table}[h!]
\footnotesize
  \centering
...
to the magnitude of residual error. (TODO symbols need to be updated)}
\label{fig:fe}
\end{center}
\end{figure}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1\textwidth]{../figs/itq-combined.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1.05\textwidth]{../figs/stan-offset-break-anti-spaghetti.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.6\textwidth]{../figs/stan-offset-break-anti-spaghetti-specializing-no-effort.pdf}
