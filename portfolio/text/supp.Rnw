...
or vessel sizes. These strategies all had at least 100 
permit holders.}
...
predicting revenue (b or $\beta$) and variability (g or $\gamma$) in revenue.
The coefficient $u_1$ represents a strategy level predictor
relating mean species diversity of a strategy
to the expected magnitude of residual error.}
\label{fig:fe}
\end{center}
\end{figure}
...
Panels refer in order to $\gamma_0$ (g0), $\gamma_1$ (g1), $\gamma_2$ (g2),
$\beta_1$ (b1), and $\beta_2$ (b2) from Eqns \ref{eq:mean} and \ref{eq:cv}.
Shown are medians (dots) and 50\% and 90\% credible intervals
