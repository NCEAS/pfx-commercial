...
cfec_holder <- cfec %>%
  group_by(p_holder, year) %>%
    summarize(total_revenue = sum(g_earn, na.rm = TRUE),
     ...
     area_permit = substr(p_fshy[1],5,5),
     area_n = length(unique(substr(p_fshy,5,5))))

ignore <- filter(cfec_holder, species_permit_n > 1 | area_n > 1)  %>%
  as.data.frame

ggplot(ignore, aes(species_permit_n)) + geom_histogram()
ggplot(ignore, aes(area_n)) + geom_histogram()
ggplot(cfec_holder, aes(species_permit_n)) + geom_histogram()
ggplot(cfec_holder, aes(area_n)) + geom_histogram()

cfec_holder <- filter(cfec_holder, species_permit_n == 1, area_n == 1) %>%
  as.data.frame

nrow(ignore)/nrow(cfec_holder)
...
p <- ggplot(filter(cfec_holder, group %in% unique(cfec_holder$group)[1:20]),
