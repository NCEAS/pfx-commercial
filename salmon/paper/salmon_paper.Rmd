---
title: 'Reduced diversity and increased specialzation of Alaska salmon fishermen:
...
Figure 1 shows that the time varying intercepts on the mean, also in [@anderson_benefits] are basically flat. These represent common fluctuations in revenue experienced by all permit holders.  
  
Figure 2 is largley illustrative, but shows the benefit of including time varying coefficients.  
  
Figure 3 shows the temporal trends, by permit, in going from 1 to 2 species fished, and how those effects have changed through time. S01A, S01E, S03A appear to be decreasing. S01K and S01M increasing. S03E pretty flat?

Figure 4 shows that temporal trends, by permit, in going from 1 to 2 species fished on the variance. From [@anderson_benefits] we'd expect diversification to redudce variance -- that seems to be the biggest signal for S01K and S01M in the years of low sockeye prices.  
...
```{r, echo=FALSE, fig.pos="placeHere", fig.cap="Time varying intercepts on estimated mean revenue, by permit."}
library(rstan)
...
```{r, fig.pos="placeHere", fig.cap="Time varying effects of adding 1 species on revenue (measured in ln dollars) for hypothetical individuals with species diversity ~ 1. This shows declining benefits of diversification for S01A, S01E, S03A, increasing (but more uncertain) benefits for sockeye oriented fisheries S01M and S01K.", echo=FALSE}
# grab original data to look at permit names
diffdat = readRDS(file="../../salmon/data-generated/salmon.rds")
years = seq(min(diffdat$year), max(diffdat$year))
permits = levels(as.factor(diffdat$permit))
