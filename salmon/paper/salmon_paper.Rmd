...
author: "Eric, Sean, Ole, Rich, Milo, Jordan, others?"
date: ''
output:
  pdf_document:
    fig_caption: yes
...
for(i in 1:nrow(df)) {
  tmp = x1[df$diversity[i]] * (est$b_1[,df$year[i], df$permit[i]]+est$b1_str_yr_mu) + x2[df$diversity[i]] * (est$b_2[,df$year[i], df$permit[i]]+est$b2_str_yr_mu)
...
ggplot(df[df$permits=="S01E",], aes(x=x,y=pred,group=years)) +
  ...
  geom_line() +
  facet_wrap(~years) + xlab("Previous species diversity") + ylab("Predicted benefit of adding +1 spp (ln $)") + geom_hline(yintercept=0, color="red")
...
for(i in 1:nrow(df)) {
  tmp = est$b0_str_yr[,b0_indx[i]] + 
    x1[df$diversity[i]] * (est$b_1[,df$year[i], df$permit[i]]+est$b1_str_yr_mu) +
...
for(i in 1:nrow(df)) {
  tmp = est$g0 + est$g0_strategy[,df$permit[i]] + x1[df$diversity[i]] * (est$g_1[,df$year[i], df$permit[i]]+est$g1_str_yr_mu) +
...
ggplot(df[df$diversity==1,], aes(x=years,y=pred,group=permits)) +
  geom_line() +
  geom_ribbon(aes(ymin=low,ymax=high), alpha=0.4) +
  facet_wrap(~permits) + xlab("Year") + ylab("Predicted marginal benefit of adding +1 spp (ln $)")
