...
pws = read.csv("../../salmon/data-generated/pws_totalHarvest.csv")
kodiak = read.csv("../../salmon/data-generated/kodiak_totalHarvest.csv")

pws = melt(pws[,c("Year","Chinook","Sockeye","Coho","Pink","Chum")], id.vars = c("Year"))
kodiak = melt(kodiak[,c("Year","Chinook","Sockeye","Coho","Pink","Chum")], id.vars = c("Year"))
pws$region = "Prince William Sound"
kodiak$region = "Kodiak"
datall = bind_rows(pws,kodiak)
names(datall)[which(names(datall)=="variable")]="species"
datall$value = datall$value / 1000000
names(datall)[which(names(datall)=="value")]="totalharvest"

# join in hatchery harvest with total harvest
dat = left_join(dat, datall)

dat$percent_hatch = dat$value/dat$totalharvest
dat$percent_hatch[is.na(dat$percent_hatch)] = 0
dat$percent_hatch[dat$percent_hatch>1] = 1
# chinook chum coho pink sock
#cbPalette <- c("#009E73", "#4B2E83", "#999999", "#CC79A7", "#D55E00")

ggplot(dat[dat$region%in%c("Kodiak", "Prince William Sound") & dat$Year < 2015,], aes(Year, percent_hatch, color=species)) +
  geom_line() +
  xlab("Year") + 
  ylab("Proportion of hatchery harvest") +
  facet_wrap(~region, scale="free_y")
#  
#  scale_fill_manual(values = c('Chinook'=cbPalette[1], 'Chum'=cbPalette[2], 
#    'Coho'=cbPalette[3], 'Pink'=cbPalette[4], 'Sockeye'=cbPalette[5]), name="Species",
