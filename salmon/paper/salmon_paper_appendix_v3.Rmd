...
seasons$permit[which(seasons$permit=="S 01A")] = "SEAK-seine"
seasons$permit[which(seasons$permit=="S 01E")] = "PWS-seine"
seasons$permit[which(seasons$permit=="S 01K")] = "Kodiak-seine"
seasons$permit[which(seasons$permit=="S 01M")] = "AKP-seine"
seasons$permit[which(seasons$permit=="S 03A")] = "SEAK-gillnet"
...
```{r figa1, fig.cap="Participation (unique people). Permits are coded by gear ('01'=seine, '03' = drift gillnet, '04' = set gillnet) and region ('A'=SEAK, 'D'=Yakutat, 'E'=PWS, 'H'=Cook Inlet, 'K'=Kodiak', 'L'=Chignik, 'M' = Alaskan Peninsula', 'T'=Bristol Bay', 'W'=Kotzebue). Effective species diversity is calculated as 1/Simpson's Diversity.", fig.pos="placeHere", echo=FALSE}
print(appendixPlot1)
```

\break  

```{r figa2, fig.cap="Diversity, mean revenue and CV (aggregated across years). Permits are coded by gear ('01'=seine, '03' = drift gillnet, '04' = set gillnet) and region ('A'=SEAK, 'D'=Yakutat, 'E'=PWS, 'H'=Cook Inlet, 'K'=Kodiak', 'L'=Chignik, 'M' = Alaskan Peninsula', 'T'=Bristol Bay', 'W'=Kotzebue). Effective species diversity is calculated as 1/Simpson's Diversity.", fig.pos="placeHere", echo=FALSE}
...
g1 = group_by(cfec.raw[cfec.raw$p_fshy=="S 01A" & cfec.raw$spec %in% c("CHUM","COHO","SOCK","PINK","CHNK"),], year, spec, p_fshy) %>%
  ...
  ylab("Species %") + xlab("") + ggtitle("SEAK-seine") + 
...
g3 = group_by(cfec.raw[cfec.raw$p_fshy=="S 01K" & cfec.raw$spec %in% c("CHUM","COHO","SOCK","PINK","CHNK"),], year, spec, p_fshy) %>%
  summarize(p = sum(g_earn)) %>% group_by(year, p_fshy) %>% mutate(p = p/sum(p), col = as.numeric(as.factor(spec))) %>%
  ggplot(aes(year, p, group=spec, color = spec)) + geom_line() +  
  ylab("Species %") + xlab("") + ggtitle("Kodiak-seine") + theme(legend.position="none") + 
scale_colour_manual(values = c('CHNK'=cbPalette[1], 'CHUM'=cbPalette[2], 
    'COHO'=cbPalette[3], 'PINK'=cbPalette[4], 'SOCK'=cbPalette[5]))
...
g4 = group_by(cfec.raw[cfec.raw$p_fshy=="S 01M" & cfec.raw$spec %in% c("CHUM","COHO","SOCK","PINK","CHNK"),], year, spec, p_fshy) %>%
  summarize(p = sum(g_earn)) %>% group_by(year, p_fshy) %>% mutate(p = p/sum(p), col = as.numeric(as.factor(spec))) %>%
  ggplot(aes(year, p, group=spec, color = spec)) + geom_line() +  
  ylab("Species %") + xlab("") + ggtitle("AKP-seine") + theme(legend.position="none") + 
scale_colour_manual(values = c('CHNK'=cbPalette[1], 'CHUM'=cbPalette[2], 
...
g5 = group_by(cfec.raw[cfec.raw$p_fshy=="S 03A" & cfec.raw$spec %in% c("CHUM","COHO","SOCK","PINK","CHNK"),], year, spec, p_fshy) %>%
  summarize(p = sum(g_earn)) %>% group_by(year, p_fshy) %>% mutate(p = p/sum(p), col = as.numeric(as.factor(spec))) %>%
  ggplot(aes(year, p, group=spec, color = spec)) + geom_line() +  
  ylab("Species %") + xlab("") + ggtitle("SEAK-gillnet") + theme(legend.position="none") + 
scale_colour_manual(values = c('CHNK'=cbPalette[1], 'CHUM'=cbPalette[2], 
    'COHO'=cbPalette[3], 'PINK'=cbPalette[4], 'SOCK'=cbPalette[5]))
...
```{r figa3, echo=FALSE, fig.pos="placeHere", fig.cap="Time varying species composition and effective species diversity (1 / Simpson's Diversity), by permit for seine fisheries in Southeast Alaska (S01A), Prince William Sound (S01E) and Kodiak (S01K). Species are colored as follows: green (Chinook), purple (chum), silver  (coho), pink (pink), red (sockeye)", fig.height=3.2}
gridExtra::grid.arrange(g1, g2, g3, h1, h2, h3, nrow=2)
```

```{r figa4, echo=FALSE, fig.pos="placeHere", fig.cap="Time varying species composition and effective species diversity (1 / Simpson's Diversity), by permit, for seine fisheries on the Alaska Peninsula (S01M), and gillnet fisheries in Southeast Alaska (S03A) and Prince William Sound (S03E).", fig.height=3.2} 
...
ggplot(df[df$diversity==1,], aes(x=years,y=pred,group=permits)) +
  ...
  facet_wrap(~permits) + xlab("Year") + ylab("Predicted marginal benefit (ln $)") + geom_hline(yintercept=0, color="red")
...
yearpermit2$permit[which(yearpermit2$permit=="S01A")] = "SEAK-seine"
yearpermit2$permit[which(yearpermit2$permit=="S01E")] = "PWS-seine"
yearpermit2$permit[which(yearpermit2$permit=="S01K")] = "Kodiak-seine"
yearpermit2$permit[which(yearpermit2$permit=="S01M")] = "AKP-seine"
yearpermit2$permit[which(yearpermit2$permit=="S03A")] = "SEAK-gillnet"
yearpermit2$permit[which(yearpermit2$permit=="S03E")] = "PWS-gillnet"

ggplot(data=yearpermit2) + 
