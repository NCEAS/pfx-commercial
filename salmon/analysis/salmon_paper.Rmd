...
- Late 1980s/early 1990s saw pretty strong declines in pink prices + high abundance (tie in with crash of foreign currencies)  
...
### Hatchery programs were designed to reduce variation in harvest. Has that happened?  
Below we quantified 3 trends in variation: year-to-year variation at the entire fishery level, year-to-year variation at the individual level, and variability between people
...
- Ability to specialize primarily driven by forces beyond control of fishermen. But in some situations, diversification is possible by extending fishing season to exploit additional spp (fishing earlier for chum in PWS S01E).  
...
# Has variability in harvest been reduced? (in part due to increased hatchery production?)

### There's a number of ways we can think about quantifying variation  
- We can calculate the aggregate (total harvest weight or revenue summed over people)  
- We can calculate the variability in revenue across individual fishermen  
- We can compare variability of revenue of individual people  

What we see here is that (1) aggregate variability remains high at the fishery level. In part because of large swings in prices in late 1990s/early 2000s. We also see (2) variability across people flat or increasing for most fisheries, with the exception of S03A. Finally, we see (3) Increased CV of revenue for people fishing in the recent decade who were also fishing 1985-1995 (exception: S01A)

```{r, fig.cap="Trends in annual variability by fishery", fig.pos="placeHere", echo=FALSE}
summary = group_by(cfec.raw[cfec.raw$spec%in%c("CHUM","COHO","CHNK","SOCK","PINK"),], year, p_fshy) %>% 
  summarize(sumg = log(sum(g_earn)), sumw = log(sum(g_pounds))) %>% 
  arrange(p_fshy, year) %>% group_by(p_fshy) %>% 
  mutate(logdiffg = rollapply(c(NA, diff(sumg)), 3, sd, fill=c(NA, NULL, NA)), 
    logdiffw = rollapply(c(NA, diff(sumw)), 3, sd, fill=c(NA, NULL, NA)))

ggplot(summary, aes(year, logdiffg )) + geom_line(color ="green" ) + facet_wrap(~ p_fshy, scale="free") + geom_line(aes(year, logdiffw), color = "blue") + ylab("3 year rolling CV") + xlab("Year") + ggtitle("Total revenue (green) and weight (blue)")
```

```{r, fig.cap="Trends in variability across people", fig.pos="placeHere", echo=FALSE, fig.height=3.5}
